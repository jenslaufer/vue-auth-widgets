<template>
  <div class="w-full max-w-md mx-auto">
    ...
    <div class="mb-4">
      <label for="firstName" class="block text-gray-700 font-semibold">{{ labels.firstName }}</label>
      <input
        id="firstName"
        type="text"
    ...
    <div class="mb-4">
      <label for="lastName" class="block text-gray-700 font-semibold">{{ labels.lastName }}</label>
      <input
        id="lastName"
        type="text"
...
const emit = defineEmits<[
  ...
  (e: 'submit', payload: { firstName: string; lastName: string; email: string; password: string }) => void
]>()
...
async function onSubmit() {
  ...
  try {
    ...
    const endpoint = props.endpoints?.register ?? '/user'
    ...
    // you can refine based on status codes
